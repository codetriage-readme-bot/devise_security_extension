...
Warden::Manager.after_set_user :only => :fetch do |record, warden, options|
  scope, session, env = options[:scope], warden.session(options[:scope]), warden.request.env
  if record.respond_to?(:accept_limitable_token?) && warden.authenticated?(scope) && options[:store] != false && !env['devise.skip_session_limitable']
