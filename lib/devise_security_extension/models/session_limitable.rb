  ...
  module Models
    ...
    module SessionLimitable
      ...
      def create_session_space!
        reject_session_on_limit? ? self.devise_sessions.destroy_all('last_request_at <= ?', (Time.now - session_expiration)).count > 0 : self.devise_sessions.order(:last_request_at).reverse_order.offset(sessions_allowed_count-1).destroy_all.count > 0
      end
